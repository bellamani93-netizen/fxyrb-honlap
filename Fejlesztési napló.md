...
## 2026.09.03. — Pontosabb chevron-grafika: "balra nyíl.png" a korábbi chevron_unit.png helyett

Marci szerint a korábbi, magunk vágta chevron-grafika nem volt elég precíz (főleg a videókiosztás nagyméretű, dupla nyilainál látszott). Lecseréltük a saját "balra nyíl.png" grafikájára, konzisztensen minden előfordulásnál (eyebrow-k, lábléc, GYT videókiosztás lime nyilai, folyamat-szekció), változatlan vizuális eredménnyel.
...
## 2026.09.04. — A "mutasd meg" lap átdolgozása: maximális méretű body chart

Marci szerint a 4. lap kritikus: a body chart (bc) legyen a lehető legnagyobb, fölötte/alatta ne legyen semmi, a gombok kerüljenek a képtől balra egymás alá, és a lapozó-sáv legyen sokkal vékonyabb — ez utóbbi minden lapon következetesen.

**Megvalósítás:** a 4. lapon eltűnt a cím/alcím, a body chart keretét a korábbi vh-becslés helyett a flexbox natív `stretch` viselkedése tölti ki teljes magasságban. A nézet-váltó, a "méret"/"hely" gombcsoportok és a "jelölés" gomb egy új, keskeny, a kép BAL oldalán álló oszlopba kerültek, mindegyik blokk egymás alatt. A lapozó-sáv paddingja/betűmérete jelentősen csökkent — mivel ez a közös `.allapotfelmero-pager*` osztályokon van, minden lapon (nem csak a 4.-en) érvényes. Emellett egy fontos pontosítás: a jelölések mostantól egy külön overlay-divbe kerülnek, aminek a CSS `mask-image`-e maga a testábra-kép — ez garantálja, hogy egy folt SOSEM lóghat túl a test kontúrjain, mert a böngésző natívan levágja a kép átlátszó részére eső darabját.

**Tesztelve böngészőben:** a bc a rendelkezésre álló magasság ~98%-át foglalta el (korábban ~40%-át). Egy fej-szélére helyezett nagy folt pontosan a hajkontúrhoz vágva jelent meg, egy teljesen a testen kívüli kattintás helyén semmi nem látszott, egy háta-közepi folt pedig teljes, vágatlan körként — a maszkolás mindhárom esetben pontosan a várt módon működött. `npm run build` hibamentes.

## 2026.09.04. — További korrekciók a "mutasd meg" laphoz és a teljes lapozóhoz

Marci pontosításai: a hát és a röntgen kép legyen pontosan ugyanakkora (az éles kontúrhoz, ne a röntgen derengéséhez igazítva), a gombok kerüljenek a képtől jobbra (nem balra), a hát/röntgen kapcsoló legyen vízszintes, alapból kikapcsolt (hát nézet), "rtg" felirattal, a "hely" mező törlődjön az összes válaszlehetőséggel, a kép talpa alá kerüljön egy szabálytalan, éles kontúrú árnyékfolt, a lenti lapozó-sáv törlődjön teljesen, helyette a fejléc aljára kerüljön egy lime, derengő haladás-sáv és 2 kerek nyíl-gomb (számláló nélkül) a lap tetején, és a két kép színeit vizsgáljam át jobb kontraszt érdekében mindkét témában.

**Megvalósítás:** Python/PIL-lel pixel-pontosan megmértem, hogy a röntgen-kép éles kontúrja (a derengés-margó nélkül) pontosan a hát-kép teljes méretével (166×529) egyezik — a röntgen-képet erre vágtam, így a két kép mostantól identikus méretű. A gombok-oszlop a kép jobb oldalára került, a kapcsoló vízszintesre és alapból kikapcsoltra váltott, a "hely" mező (típus, adat, UI) teljesen törölve. Új `FootShadow` SVG a talp alá. A lenti lapozó-sáv (progress + gombok + számláló) helyett egy fenti sáv: lime, derengő haladás-csík közvetlenül a fejléc alatt, alatta 2 kerek chevron-gomb (az utolsó lapon pipa-ikonos "mentés" gombra váltva a jobb oldali). A kontraszt-problémát (a röntgen-kép sötétkék kitöltése sötét módban majdnem eltűnt a navy háttérben) egy halvány, téma-függő "színpad"-háttérrel és mindkét képre alkalmazott, téma-függő `drop-shadow` rim-derengéssel oldottam meg — a forrás PNG-k pixelei nem változtak, minden téma-adaptáció CSS-ben történik.

**Tesztelve böngészőben:** világos és sötét módban is — a két kép váltáskor pixelre azonos méretű/pozíciójú, a gombok jobbra, a kapcsoló vízszintes és alapból ki, "rtg" felirattal, a "hely" szekció eltűnt, a talp alatt jól látható árnyékfolt van, a fenti lime sáv és a 2 kerek gomb minden lapon (nem csak a 4.-en) konzisztensen megjelenik, számláló nélkül. Sötét módban mindkét kép jól elvált a navy háttértől. Egy apró hibát menet közben javítottam: a "mentés" gomb pipa-ikonja kezdetben a gomb saját hátterébe olvadt (azonos szín) — most explicit kontraszt-színt kapott. `npm run build` hibamentes.

## 2026.09.04. — "Mutasd meg" lap: telefonos nézet átdolgozása szabadkéz-rajzolással

Marci kérésére, KIZÁRÓLAG telefonos nézetben (az asztali elrendezés változatlan): az árnyékfolt nagyobb lett és a kép saját méretéhez (nem a szélesebb keretéhez) igazodik; a nézet-kapcsoló fölé "nézet" felirat került, alá "jelöld be" + egy kerek "+" gomb; a "+" gombra kattintva egy középre nyíló popup jelenik meg ("tünet mérete" cím, egy sorba férő pontszerű/kicsi/nagy váltó, felszólítás, "rajzolás" gomb); a "rajzolás" gomb bezárja a popupot és aktiválja a rajzolást; ha van már jelölés, egy "visszavonás" nyíl jelenik meg a legutóbbi törlésére.

**A legnagyobb változás:** a jelölés mostantól nem csak koppintással (pont), hanem HÚZÁSSAL (szabadkézi vonal) is felvehető. A jelölések adatmodellje pontból pontsorra (stroke) alakult át, Pointer Events-alapú kezeléssel — de a húzás közbeni pontgyűjtés kifejezetten csak telefonos nézetben aktív (`window.matchMedia`-alapú ellenőrzéssel), asztalon a régi "egy kattintás = egy pont" viselkedés maradt. A jelölések renderelése egy közös, maszkolt SVG-rétegre váltott (pontok és vonalak is, mindkettő a testábra kontúrjához vágva, elmosott széllel), az egyedi jelölésre kattintós törlést a "visszavonás" gomb váltotta fel (mindkét nézetben).
...
## 2026.09.04. — Marci saját SVG-grafikái, tartalmi finomítások, kalkulátor teljes képernyős lapja + egy valós regresszió javítása

Marci feltöltötte a hát és a röntgen kép saját, kézzel rajzolt SVG-forrásait (a korábbi pixeles PNG-k helyett), bennük egy közös referencia-árnyékkal. Mindkét SVG saját, egymástól független rajzvászon-koordinátákkal készült, ezért egy Python-szkript a két fájl árnyék-elemének pontos helyét (`getBBox()` + `getCTM()`-transzformáció a lokális→root koordinátákhoz) megkeresve, közös viewBox-ot számolt és egy `<g transform="translate(0,DY)">`-be csomagolta mindkét fájl tartalmát — így nézetváltáskor az árnyék (és vele az egész alak) pixelre pontosan ugyanott jelenik meg.

További változtatások: a pontszerű koppintás elhalványulása visszaállítva az eredeti (a legutóbbi körben tévesen a vonalra ÉS a pontra is alkalmazott, vonalhoz igazított) beállításra; az üdvözlő lap ikonja lime hátterű villanykörtére váltott; a lapozó-gombok külön fejléc-sávja megszűnt, szabadon lebegnek; "Tünet: mit érzel?" címke + új kitöltő szöveg; az intenzitás-csúszka a kalkulátoréval azonos megjelenésű egyedi csúszkára váltott (alapérték 0-ra); a "mikor kezdődött?" mező legördülő menüre váltott; "Személyes célod" cím került a személyes cél mező fölé; a gerincterhelés kalkulátor lapja mostantól a teljes képernyőt tölti ki (csak a lime csík + lebegő "vissza" gomb marad látható felül), és a "továbblépés" pipa-gomb csak akkor jelenik meg, ha a kitöltő csík elérte a 24/24 órát.

**Valós hiba a böngészős tesztelés közben:** az utolsó pont (24 órás gate) bevezetése egy komoly regressziót okozott — bármelyik csúszka mozgatásakor a teljes kalkulátor kiürült (minden sor/csúszka eltűnt, minden érték 0-ra ugrott). Kiderítve: a kalkulátor a saját tartalmát `dangerouslySetInnerHTML`-lel egyszer beszúrja, utána egy `useEffect` NYERS DOM-manipulációval építi fel a sorokat, amiről React "nem tud" — amikor egy csúszka-mozgatás a szülő állapotát (a 24 órás gate-hez bevezetett `calcHours`) megváltoztatta, a szülő újrarenderelt, és ez visszaállította a kalkulátor `dangerouslySetInnerHTML`-jét az üres kiinduló sablonra, annak ellenére, hogy a kalkulátor saját propja nem változott. Javítás: a komponens `React.memo()`-ba csomagolva, hogy a szülő újrarenderelése egyáltalán ne érintse.
...
## 2026.09.04. — Hibajavítás: a kalkulátor eleje nem látszott asztali nézetben
...
## 2026.09.04. — Hibajavítás: osztálynév-ütközés a Bootstrap saját ".row" rács-osztályával
...
## 2026.09.08. — Eredménylap újraépítve "terv.jpg" kézi vázlat alapján

Marci feltöltött egy kézzel rajzolt vázlatot a kívánt asztali elrendezésről, és kérte, hogy csak a teljes megértés után kezdjek építeni. A képet elemezve 4 kérdést tettem fel: a BMI+Gerincterhelés+Aktivitási szint visszakerüljön-e egy közös ("Életmód") dobozba; a testábra kikerüljön-e a Tünet dobozból, önálló nagy illusztrációként; és az Intenzitás/Időtartam dial-stílusú legyen-e a jelenlegi sáv/kördiagram helyett — mindegyikre igen volt a válasz.

A legnagyobb technikai kihívás: mivel a testábra és az Intenzitás/Időtartam KOMPONENSTÍPUSA is változik asztalon (nem csak a pozíciója), a puszta grid-area-áthelyezés nem volt elég — egy dobozból elemet kiemelni csak a JSX-ben lehet, ami mobilt is érintené. Megoldás: a testábra és a 2 dial JSX-e egyszer van megírva, de kétszer kerül a DOM-ba (egy mobil-only és egy desktop-only változatban), és egy új `.eredmeny-mobile-only`/`.eredmeny-desktop-only` osztálypár dönti el CSS-ből, melyik látszik.
...
## 2026.09.09. — Asztali elrendezés finomhangolása: fejléc-cím törölve, 3 független magasságú oszlop, testábra feljebb

Marci 7 pontos pontosítást kért, még mindig az 1. fázison (asztali nézet, görgetés nélkül) belül: törölni az "eredményeim" címet; a testábrát és az alatta lévő tartalmakat feljebb hozni (nem kell fölé egy nagy infó-doboz); az Életmód doboz se legyen kényszerítve a Rizikó/Mozgékonyság sorával egy magasságba; a Történet és Életmód dobozcímek kövessék a többi doboz formázását; a nézet-kapcsoló kerüljön a testábra mellé, jobbra, a lábánál; az Intenzitás dial-nál ne legyen külön szöveges magyarázat; és a dobozokon belüli elválasztó vonalak mindig dobozhatárig érjenek.

A legnagyobb technikai váltás: a korábbi CSS Grid rács nem tudott oszloponként FÜGGETLEN magasságot adni (minden sor a legmagasabb elemhez igazodott) — ezért lecseréltem egy 3 oszlopos Flexbox-elrendezésre, ahol mindegyik oszlop a saját tartalmának megfelelő, egymástól független magasságra nő. Mivel ez a DOM-ban is ténylegesen átcsoportosítja a kártyákat, ami mobilon felborította volna a megszokott egyoszlopos sorrendet, egy `display:contents` + CSS `order` kombinációt vezettem be: az oszlop-wrapperek mobilon "eltűnnek" a doboz-modellből, a bennük lévő kártyák egyenesen a mobil flex-lista gyerekei lesznek, a saját `order` értékük szerint besorolva — ez mindkét (mobil és asztali) elrendezésben helyesen működik. Az "eredményeim" cím helyén egy kompakt, doboz nélküli fejléc-összegzés jelenik meg (név/kor/magasság/kitöltés dátuma), az Alapadatok kártya emiatt mostantól csak mobilon látszik, normál (nem headerless) formában.

Egy hibát találtam és ELSŐRE hibásan javítottam, majd méréssel korrigáltam: a "dobozhatárig érő elválasztó vonal" kérést először minden info-sorra negatív margóval próbáltam megoldani — DOM-méréssel kiderült, hogy ez feleslegesen (a Történet kártya sorai már enélkül is pontosan dobozhatárig értek) sőt károsan hatott (a szegélyt a doboz szélén TÚLRA tolta). A valódi eset csak a Tünet doboz "Jól esik"/"Trigger" sorpárja volt, ahol a szülő elem (nem maga a doboz) adott paddingot — a javítást erre az egy helyre szűkítettem.

**Tesztelve böngészőben:** a teljes 10 lépéses kérdőívet szkriptelve kitöltve, majd az Eredménylapra navigálva — 1440×900, 1366×800 és 1200×800px szélességben méréssel megerősítve, hogy nincs görgetés; a 3 oszlop ténylegesen eltérő magasságú; a testábra és a nézet-kapcsoló alsó éle egy vonalban van; az Intenzitás dial-nál nincs kategória-szöveg, az Időtartamnál van. Sötét módban is megerősítve a helyes megjelenés és a görgetés-mentesség. Mobilon DOM-méréssel megerősítve, hogy mind a 9 kártya a régi sorrendben, a régi formában jelenik meg — a mobil nézet pixelre érintetlen maradt. `npm run build` hibamentes.

## 2026.09.09. — Asztali elrendezés finomhangolása, 2. kör: egységes dobozcím-stílus, keskenyebb közép oszlop, fejléc átrendezve

Marci 6 pontos pontosítást kért, még mindig az 1. fázison belül: a Történet/Célod/Életmód dobozcímek kövessék a többi (pl. Tünet) doboz formázását; a Gerincterhelés/Aktivitási szint kijelzők legyenek egymagasságban; a Mozgékonyság és Célod doboz lehet keskenyebb, hogy több hely jusson a Történet/Életmód dobozoknak; a kitöltés időpontja maradjon a jobb felső sarokban; a kor/magasság kerüljön a név/becenév alá; és a dobozok külső élei igazodjanak egymáshoz a különböző képernyőméreteken.

Mivel EZUTÁN minden doboz-cím ugyanazt az egységes, nagy/hangsúlyos stílust kapja, a korábbi "kiemelt/másodlagos" 2-féle cím-stílus (és a hozzá tartozó `SectionCard` `prominent` prop) feleslegessé vált — töröltem, dead code volt. A közép oszlop 6/12-ről 4/12-re szűkült, a jobb oszlop 3/12-ről 5/12-re bővült (a testábra saját mérete ettől nem változik, csak a Mozgékonyság/Célod dobozok keskenyedtek). A fejléc-összegzésben a kor/magasság a névsor alá került, a kitöltés dátuma pedig egy `justify-content:space-between` elrendezéssel mindig a sáv jobb szélén marad.

Két valós hibát találtam és javítottam, mindet méréssel: a kitöltés-dátum nem ért a lap jobb széléig, mert a fejléc-összegzés doboza csak a saját tartalma szerinti szélességet vette fel (`flex:1`-gyel javítva); és a közép oszlop szűkítése után a Mozgékonyság 4 jelvénye nem fért ki egy sorban a legszűkebb (992px) szélességen, ami 1200×800px-en látható görgetést okozott — a jelvények méretét a legszűkebb esetre újraméreteztem, hogy mindig egy sorban maradjanak.

**Tesztelve böngészőben:** a kérdőív kitöltése közben kiderült, hogy a "beküldés" gomb JavaScript `.click()`-je megbízhatatlan (a navigáció tévesen a Főoldalra futott ki, elveszítve a kitöltött állapotot) — a böngésző `computer` eszközének natív kattintására váltva ez megszűnt. 992×800, 1200×800, 1366×800 és 1440×900px szélességben megerősítve, hogy 1200px+-on nincs görgetés (992px-en egy kicsi, ~12px-es, korábban is elfogadott maradék görgetés mérhető); a Gerincterhelés/Aktivitási szint dial-ok pixelre egyező magasságban indulnak; minden doboz-cím egységes méretű. Sötét módban és mobilon (375px) is megerősítve, hogy minden a várt módon jelenik meg, a mobil nézet érintetlen maradt. `npm run build` hibamentes.

## 2026.09.09. — Asztali doboz-viszonyok rögzítése: egyenlő szélesség, alsó-igazítás

Marci egy általános szabályt fogalmazott meg, ami felülírja az előző kör 3/4/5 arányú oszlop-szélességét: a dobozok magassága maradjon dinamikus, de a SZÉLESSÉGÜK legyen mindig egyenlő; a Rizikó, Célod és Életmód doboz (a 3 oszlop utolsó kártyája) alja legyen egy vonalban; a cél mindig a képernyő maximális, de szabálykövető kitöltése.

A megvalósítás 3 összefüggő CSS-szabály: a 3 oszlop mindegyike egyenlő flex-arányt kapott (minden doboz egyenlő széles lett); a 3 oszlopot tartó sor `align-items:stretch`-re váltott, hogy mindegyik oszlop a legmagasabb (a testábrás közép) oszlop magasságára nyúljon; és minden oszlop UTOLSÓ kártyája (pontosan Rizikó/Célod/Életmód) `margin-top:auto`-t kapott, ami a stretch által keletkezett extra helyet a kártya FÖLÉ tereli — így ez a 3 doboz mindig együtt zár alul, a felettük lévő dobozok pedig változatlanul a saját tartalmuk szerint méreteződnek.

Egy valós hibát találtam és javítottam: az Életmód doboz 2 alsó dial-ja a jobb oszlop szűkülése után a legszűkebb (992px) szélességen pár pixellel túllógott a doboz saját szélén — ami sértette volna az imént rögzített "egyenlő szélesség" szabályt. A dial-ok közti rés csökkentésével (a Mozgékonyság-jelvényeknél korábban bevált mintát követve) megoldva.

**Tesztelve böngészőben:** a teljes kérdőívet szkriptelve kitöltve — DOM-méréssel megerősítve, hogy 1440×900px-en mind a 3 oszlop szélessége pontosan egyezik, és mindhárom oszlop (és a Rizikó/Célod/Életmód dobozok) alja pontosan egy vonalban zár. Nincs görgetés 1200px+ szélességeken. Sötét módban és mobilon (375px) is megerősítve, hogy minden a várt módon jelenik meg — a mobil nézet érintetlen maradt (az új `margin-top:auto` szabály kizárólag az asztali nézetben fut). `npm run build` hibamentes.

## 2026.09.09. — Tünet doboz növekedése, Időtartam-felirat törlése, neon mutató-színek, Aktivitási szint egysoros felirat

Marci 4 pontosítást kért: ha a Rizikó dobozban kevés az elem, a Tünet doboz nőjön meg (belül a leírás és a mutatók közötti térköz nagyobb legyen); az Időtartam mutató alatti kisbetűs érték-jelzőt vegyem ki (ugyanúgy, ahogy az Intenzitásnál korábban történt); a mutatók színskálája legyen neon, jó kontraszttal világos ÉS sötét háttéren is; és az Aktivitási szint mutató fölötti szöveg maradjon egy sorban, elég térközzel a két mutató között.

A Tünet doboz növekedését a már bevált auto-margós technikával oldottam meg, eggyel beljebb alkalmazva: a bal oszlop első kártyája (a Tünet) most maga nő meg (`flex:1 1 auto`), a doboz belsejében pedig a leírás és a dial-sor közötti rés nyeli el a felszabaduló helyet (`margin-top:auto` a dial-soron) — így a Rizikó doboz, ha rövid, nem hagy üres rést maga előtt, hanem a Tünet doboz duzzad meg, miközben az oszlop alja (és vele a Rizikó/Célod/Életmód közös alsó vonala) változatlan marad.

A neon-színekhez a projekt már meglévő, globális `--z1..z6`/`--a1..a5` zóna-színskáláját (amit a Gerincterhelés-kalkulátor ÉS az Eredménylap minden mutatója közösen használ) cseréltem le — WCAG relatív luminancia-számítással kerestem meg azt a szűk tartományt, ahol egy szín EGYSZERRE ≥3:1 kontrasztos a fehér ÉS a sötét doboz-háttérrel is, majd ebbe a sávba illesztettem 6, egymástól jól megkülönböztethető, telített (piros→narancs→borostyán→türkiz→zöld→smaragd) tónust. Mivel ez egyetlen, központi tokenkészlet, a csere a kalkulátor saját (wizard-beli) megjelenítésére is automatikusan érvényes — ez tudatos döntés volt az egységesség mellett.

Az Aktivitási szint egysoros feliratához az Életmód doboz alsó sorának 2 dial-ját a korábbi fix (105px-es) szélesség helyett rugalmasra állítottam — ez egyszerre oldja meg az előző kör túllógási hibáját (fizikailag kizárt) és ad elég helyet a kétszavas feliratnak a tipikus (≥1200px) szélességeken; csak a legszűkebb (992px) nézetben marad 2-soros, elfogadott szélső esetként.

**Tesztelve böngészőben:** a kérdőívet szándékosan 0 kiválasztott rizikó-tétellel töltöttem ki, hogy a "kevés elem" esetet reprodukáljam — DOM-méréssel megerősítve, hogy a Rizikó doboz minimális magassággal jelenik meg, mégis pontosan a Célod/Életmód dobozok aljával egy vonalban zár, mert a Tünet doboz nőtt meg (a leírás és a dial-sor közötti rés 227px-re nőtt, a korábbi ~8px helyett). A dial-feliratok tényleges sortörését (nem a konténer, hanem a szöveg saját sorai szerint) ellenőrizve: 1440px és 1200px szélességben mindkét felirat egy sorban van; 992px-en csak "Aktivitási szint" törik 2 sorra, vízszintes túllógás nélkül. A neon-színeket világos és sötét módban screenshot-tal is megerősítve — mindkét háttéren jól látszanak. Mobilon (375px) megerősítve, hogy a Tünet doboz `display:block` maradt (a növekedés-szabály kizárólag asztalon fut), és a neon-színek a mobil mutatóknál is helyesen jelennek meg. `npm run build` hibamentes.

## 2026.09.09. — Neon mutató-színek pontosítása: narancs/zöld/kék élénkebb

Marci megnézte az előző kör visszafogott neon-palettáját, és pontosított: a piros jó, de a narancssárga és a zöld/kék legyen élénkebb. A piros és a borostyán tónus változatlan maradt; a narancs és a türkiz/zöld/smaragd tónusok magasabb relatív luminanciájú, élénkebb változatra váltottak.
...
## 2026.09.10. — Eredménylap 3. fázis indul: nyomtatható A/4 összegzés

A mobil nézet lezárult. Marci nyitott, feltáró kérdést tett fel: hogyan lehetne az Eredménylapot egy álló A4 lapra, nyomtatóbarát módon elhelyezni. Mivel ez nem egy pontosan körülírt utasítás volt, a nekiállás előtt egy rövid javaslatot fogalmaztam meg (dedikált `@media print` stílus a meglévő oldalon, nem külön route/komponens), amit Marci jóváhagyott, mielőtt elkezdtem az építést.

A megközelítés: böngésző-natív nyomtatás egy új "nyomtatás" gombbal, `@page` A4-méret + margó, és egy önálló `@media print` szabálykészlet — nem a képernyős asztali médialekérdezésre támaszkodva, mert a nyomtatás közben a "szélesség" nem garantáltan egyezik egyik képernyős töréspont szélességével sem. Felismertem, hogy az A4 nyomtatott oszlopszélesség gyakorlatilag megegyezik a már bevált, legszűkebb asztali eset méreteivel, így a kompakt dial-/jelvény-méretezést onnan, újratervezés nélkül vettem át — ez jelentősen csökkentette a kockázatot.

Három tervezési döntés: a nyomtatás mindig világos háttéren történik, függetlenül a képernyőn beállított témától (egy pár CSS-változó felülírásával, ami minden rá épülő szabályt automatikusan követ) — a mutatók jelentéssel bíró neon-színei viszont megmaradnak, mert nyomtatva is informatívak; az interaktív nézet-kapcsoló helyett egy statikus felirat mutatja a kiválasztott testábra-nézetet; és a testábra nyomtatáskor oszlop-elrendezésű (nem a képernyős sor-elrendezés, ami az interaktív kapcsolóhoz készült).

A böngésző natív nyomtatási előnézete egy OS-szintű felület, amit az automatizált böngésző-eszköz nem tud közvetlenül megnyitni vagy lefényképezni — ezt egy ideiglenes technikával kerültem meg: a print-szabályokat egy élő oldalba injektált, médialekérdezés nélküli `<style>`-taggel teszteltem egy A4-arányos nézetablakban, ami lehetővé tette a navigációt is a teszt alatt. Egy finomítást végeztem a mérés után: a testábra méretét megnöveltem, hogy jobban kitöltse a rendelkezésre álló lapot, még mindig bőséges tartalékkal.

**Tesztelve böngészőben:** a teljes kérdőívet szkriptelve kitöltve, éppen sötét módú munkamenetben — ez véletlenül tökéletes valós tesztje volt a "mindig világos nyomtatás" szabálynak. Az injektált print-stílussal screenshot-tal megerősítve, hogy mind a 6 doboz egyetlen, jól olvasható A4 oldalra fér, a mutatók színesek maradnak, a háttér mindenhol fehér. DOM-méréssel megerősítve, hogy a tartalom magassága jóval az A4 használható magasság alatt marad. A normál képernyős nézetben (mobil és asztali, világos és sötét módban) is megerősítve, hogy az új gomb és felirat nem okoz regressziót. `npm run build` hibamentes.

## 2026.09.10. — Eredménylap láthatóvá tétele a GYT fiókban, "állapotfelmérők" menüpont

A 3 fázisban felépült Eredménylap ezzel lezárult. Marci egy új, ehhez kapcsolódó funkciót kért: az ÜF-hez rendelt Eredménylap legyen látható a GYT fiókban is, egy új "állapotfelmérők" menüpont alatt — és a nyomtatás opció kizárólag a GYT fiókban maradjon, az ÜF sajátjából törölve.

Mivel a projektnek nincs backendje, a megoldás nem hozott létre új, ügyfél-specifikus adatréteget: az Eredménylap komponense továbbra is a közös, munkamenet-szintű állapotfelmérő-adatból olvassa az összes tartalmi mezőt (Tünet, mutatók, Mozgékonyság, Történet, Életmód, Célod) — ez a GYT-oldali megjelenítésnél is ugyanaz marad, hiszen ugyanaz a komponens fut mindkét helyen. Az egyetlen ténylegesen ügyfél-specifikus adat, a fejlécben látható név, egy új, opcionális propon keresztül érkezik a valódi (már meglévő) ügyfél-nyilvántartásból; ha nincs megadva, az ÜF saját oldala változatlanul a munkamenet-névre esik vissza. Egy második, szintén opcionális prop vezérli a nyomtatás-gomb megjelenítését — alapból kikapcsolva (ÜF), a GYT-oldali új nézet mindig bekapcsolva hívja.

Az új GYT-oldali belépési pontot a már bevált "válassz ügyfelet előbb" mintára építettem (ugyanaz, amit a videókiosztás oldal is használ): ha nincs kiválasztott ügyfél, egy magyarázó felirat és egy ügyfél-listához vezető gomb jelenik meg; egyébként a kiválasztott ügyfél valódi nevét a nyilvántartásból kikeresve adja tovább. A GYT nav-menü korábban zárolt "állapotfelmérő" helyőrzőjét feloldottam, többes számúra átneveztem, és a többi aktív menüpont mellé soroltam át.

**Tesztelve böngészőben:** `npm run build` hibamentes az összes érintett fájl módosítása után. A teljes kérdőívet szkriptelve kitöltve (a "beküldés"/"következő lap" gombok `aria-label`-alapú kiválasztásával, a mezőkitöltő segédfüggvényt egy `<textarea>`-ágazattal bővítve, mert 2 wizard-lépés textarea-t használ). Megerősítve, hogy az ÜF saját oldalán nincs nyomtatás-gomb és a munkamenet-név látszik változatlanul; a GYT új oldalán van nyomtatás-gomb és a valódi ügyfél-név látszik. Mobilon és sötét módban is megerősítve screenshot-tal, hogy a GYT-oldali nézet helyesen, regresszió nélkül jelenik meg. Konzol-hiba nem jelentkezett. `localStorage` törlése megerősítve.

## 2026.09.10. — Nyomtatási hibajavítás: a dial-párok kilógása a dobozból, kisebb lapmargó

Marci a nyomtatott A/4 összegzést megnézve hibát jelzett: a mutatók kilógnak a dobozból nyomtatáskor, kért egy kisebb lapmargót, és hogy a tartalom mindig maradjon a dobozon belül.

DOM-méréssel kiderítettem a valódi okot: a Tünet doboz Intenzitás/Időtartam dial-párja fix szélességű volt, középre igazítva — amikor a 2 műszer együttes szélessége szélesebb, mint a doboz ténylegesen rendelkezésre álló belső helye, a túlnyúló rész láthatóan kilóg a doboz szegélyén. Ez NEM csak nyomtatási hiba volt: méréssel megerősítve minden töréspontnál fennállt (mobilon a legnagyobb mértékben, nyomtatva a legkisebb mértékben) — a nyomtatott lap éles, nagyított részletei miatt tűnt fel Marcinak elsőként.

A javítás ugyanazt a már bevált technikát alkalmazza, amit az Életmód doboz alsó dial-párja korábban kapott: a 2 műszer fix szélesség helyett rugalmasan, egyenlő arányban osztozik a ténylegesen rendelkezésre álló helyen — ez fizikailag kizárja a túlnyúlást, függetlenül a doboz aktuális szélességétől. A szabályt a nyomtatási CSS-blokkban is külön meg kellett ismételni (mert az önálló marad, semmit nem örököl a képernyős szabályokból), és eközben kiderült, hogy az Életmód dial-párjának MÁR MEGLÉVŐ rugalmas szabálya is csendben felülíródott nyomtatáskor egy magasabb prioritású fix-szélesség szabály által — ez egy második, eddig észrevétlen nyomtatási hiba volt, amit ugyanez a javítás megoldott.
...
## 2026.09.11. — Hosszú Előzmények-válasz tesztelése, majd a "Mikor kezdődött?"/"Volt már korábban is?" feltételes elrejtése

Marci megkért, hogy teszteljem le: mi történik, ha valaki több soros választ ad az Előzmények mezőre, hogyan változik ettől az Eredménylap asztali elrendezése. Egy hosszú, kötőjeles vázlatpontos teszt-válasszal 2 problémát találtam: a sortörések elvesznek (a böngésző egybefolyó bekezdéssé alakítja a mezőt), és a Történet doboz magasság-növekedése — mivel a 3 oszlop mindig a legmagasabbhoz igazodik — a másik 2 oszlopban is nagy, üres réseket okozott, és az oldal a viewport fölé nőtt (görgetés jelent meg olyan nézetben is, ahol korábban sosem).

Marci egy megoldást javasolt: mivel a "Mikor kezdődött?" és "Volt már korábban is?" mezők tartalmilag átfednek egy részletes Előzmények-válasszal, hosszú válasz esetén el lehetne rejteni ezt a 2 mezőt, rövid válasz esetén viszont fontos, hogy látszódjanak. Miután egyeztettünk a tervről, kértem, hogy én számoljam ki és teszteljem le a karakterszám-küszöböt, amitől az elrejtés életbe lép.

A küszöböt böngészős méréssel kalibráltam, nem becsléssel: egy automatizált teszt-hurok különböző hosszúságú szövegekkel töltötte ki a mezőt, minden alkalommal megmérve a tényleges görgetés mértékét a projekt saját, korábbról ismert kalibrációs nézetablakaiban (992×800, 1200×800, 1440×900). A mérés megerősítette, hogy a "Mikor kezdődött?"+"Volt már korábban is?" 2 sor együttes magassága kb. 130px — ennyi hely szabadul fel elrejtésükkor. 150 karakteres küszöböt választottam: ennél a hossznál az elrejtés éppen akkor lép életbe, amikor nélküle a görgetés már jelentőssé válna — 1440×900-on ez a küszöb egészen 300 karakterig görgetés-mentes marad. A legszűkebb (992×800) nézetablakban a javulás jelentős, de nem tökéletes — ez a nézetablak korábban is ismert, elfogadott szűkösségével függ össze, nem új probléma.

**Tesztelve böngészőben:** az automatizált mérés-sorozat mindhárom nézetablakban lefutott az implementáció után is, megerősítve a számokat. Egy valósághű, kötőjeles vázlatpontos válasszal screenshot-tal is megerősítve 1440×900-on, világos és sötét módban: a 3 oszlop magassága egyforma, nincs üres rés, nincs görgetés. Rövid válasszal megerősítve, hogy a küszöb alatt változatlanul mindkét mező látszik. Mobilon is konzisztens a viselkedés. `npm run build` hibamentes.

Nyitva maradt, ebben a körben nem javított megfigyelés: a sortörések elvesztése továbbra is fennáll — ez egy külön, még el nem döntött kérdés, amit Marci még nem kért megoldani. (Később, ugyanaznap: Marci lezárta — nem kell a vázlatpontokat külön ábrázolni, a jelenlegi megjelenítés marad.)

## 2026.09.11. — Óra-megoszlás körsávdiagram popup a Gerincterhelés/Aktivitási szint mutatókhoz

Marci egy új interaktív funkciót kért: a Gerincterhelés vagy Aktivitási szint mutatóra kattintva nyíljon meg egy popup, ami körsávdiagramon (donut chart) mutatja, hogyan oszlik meg a napi 24 óra a kitöltött tevékenységek között, és egy szeletre kattintva jelenjen meg a %, a tevékenység neve, az időtartam, és 2 sáv, ami megmutatja, mennyivel járul hozzá az adott tevékenység a gerincterheléshez és az aktivitási szinthez. Mivel a kérésben volt 2 érdemi tervezési kérdés (a szeletek bontása: egyedi tevékenységenként vagy 5 fő kategóriába összevonva; és a hozzájárulás-sávok skálázása), előre rákérdeztem, mielőtt nekiálltam — Marci mindkét esetben az ajánlott opciót választotta (egyedi tevékenységek, illetve a fő mutatók -20..20/0..25 tartományával megegyező skálázás).

A megvalósítás nem nyúlt a kalkulátor eredeti számítási logikájához (Marci korábbi, kifejezett kérése szerint ez érinthetetlen) — csak egy már meglévő bejáráshoz (ami eddig is összegezte az óránkénti gerincterhelés/aktivitás-hozzájárulást) adtam hozzá egy új, exportált adatsort: minden kitöltött tevékenység saját hozzájárulása, amit a kalkulátor eredmény-objektuma most már visszaad.

A körsávdiagramot egy egyszerű SVG-technikával építettem (több egymásra rakott kör, mindegyik a saját arányos ívhosszával és elforgatásával) — ez egyszerűbb, mint a kézzel számolt ív-útvonalak, és minden szelet önmagában kattintható DOM-elem. A szín-hozzárendelés futásidejű, HSL-forgatással, hogy bármennyi kitöltött tevékenységhez garantáltan annyi, jól megkülönböztethető szín tartozzon. A hozzájárulás-sávok ugyanazzal a képlettel helyesen kezelik mind a szimmetrikus (gerincterhelés), mind a nullától induló (aktivitási szint) tartományt.

**Tesztelve böngészőben:** a teljes kérdőívet kitöltve, a kalkulátor "Példa nap" gombjával (9 különböző tevékenység, jó tesztadat). DOM-méréssel pontosan megerősítve az SVG-geometriát és a részlet-panel minden adatát — kézzel visszaszámolva egyezett a kalkulátor saját szorzóival. A szeletre és a jelmagyarázatra kattintás is működik, a bezárás mind a gombbal, mind a háttérre kattintva. Mobilon és sötét módban a számított CSS-értékekkel ellenőrizve (a böngésző-panel ebben a körben nem tudott képernyőképet készíteni egy host-oldali ablakállapot-hiba miatt, ami nem az alkalmazás hibája). Konzol-hiba nem jelentkezett, build és típusellenőrzés hibamentes.

## 2026.09.11. — Óra-megoszlás popup átdolgozása: gyűrű+jelmagyarázat egymás mellett, kihúzott+derengő kiválasztás

Marci egy mintaképet küldött (egy energiamix-donut widget screenshotja) az előbb elkészült popup helyett kért elrendezésről. Átvettem a mintakép 3 fő mintázatát: a gyűrű és a jelmagyarázat egymás mellett (nem egymás alatt, kis dobozban); a kiválasztott szelet kihúzva+derengéssel emelkedik ki a gyűrűből (nem vastagabb vonallal, ahogy korábban); és a gyűrű közepén a kiválasztott tétel nagy százaléka+óraszáma jelenik meg, a jelmagyarázat megfelelő sora pedig háttér-kiemelést kap.
...
## 2026.09.15. — Állapotfelmérő: folytonos fájdalom-színskála, azonnali bodychart-rajzolás, toll-ikon, lapváltáskori görgetés-visszaállítás

Marci visszatért a kérdőívhez, 4 finomítást kérve egyszerre: a fájdalom-intenzitás csúszka színe folyamatosan váltson zöldből sárgán-narancson-piroson át sötétvörösbe, és a szám is ugyanezt a színt vegye fel; a testábránál rögtön lehessen rajzolni (ne kelljen előbb a "+" gombot és egy popupot használni); az alapértelmezett rajzolás-méret "kicsi" legyen; a "+" ikon cserélődjön tollas ikonra; és minden lap felülről induljon, még akkor is, ha az előző lapon lejjebb görgettem.
...
## 2026.09.16. — Állapotfelmérés beküldése után azonnali navigáció, minden kitöltés külön, böngészhető előzmény-bejegyzésként

Marci jelezte: a beküldés után nem navigál az oldal az Eredményeim lapra, és egy újabb kitöltés felülírná a jelenlegi eredményt ahelyett, hogy újat hozna létre. Mivel ez jelentősebb szerkezeti kérdéseket vetett fel, 2 pontosító kérdést tettem fel: legyen-e az előzmény böngészhető lista, vagy csak a legújabb látsszon? Marci a böngészhető listát választotta. Kell-e egy új, dedikált "új kitöltés" gomb, vagy elég a meglévő oldalsáv-menüpont? Marci szerint elég a meglévő menüpont.

Az `Allapotfelmero.tsx` beküldés-kezelője mostantól a végleges mentés után azonnal a `/eredmenyeim` lapra navigál. A Context egy új, "legújabb elöl" tömböt kapott minden korábban beküldött kitöltés pillanatképével — a beküldés a piszkozatot ebbe a tömbbe másolja, majd a piszkozatot alaphelyzetbe állítja, hogy egy új kitöltés valóban tiszta lapról induljon. Az Eredménylapon egy új választó-komponens jelent meg (ott, ahol eddig a dátum állt): 1 bejegyzésnél változatlanul egyszerű szöveg, 2+ bejegyzésnél egy legördülő, amivel a teljes lap tartalma a kiválasztott archív bejegyzésre vált.

A böngészős tesztelés során egy valós hibát fedeztem fel: az első implementáció egy setState-hívást egy másik setState frissítő függvényébe ágyazott, amit React fejlesztői módban (StrictMode) kétszer hív meg tisztaság-ellenőrzésként — emiatt egyetlen beküldés két bejegyzést hozott létre. Javítottam: egy ref tartja a legfrissebb piszkozatot, a 2 setState-hívás egymástól függetlenül, nem egymásba ágyazva fut le.

**Tesztelve böngészőben:** megerősítettem, hogy 1 beküldés pontosan 1 bejegyzést hoz létre (a hiba javítása után), majd egy második, eltérő adatokkal kitöltött beküldés után a választóban pontosan 2 opció jelenik meg, alapból a legújabb kiválasztva, és mindkét irányban helyesen vált a lap tartalma. Ugyanezt mobil nézetben, sötét módban és a nyomtatási DOM-szerkezetre is ellenőriztem. A GYT-oldali nézetet is átnéztem, nem érintette a változás. Melléktermékként egy munkamenet közbeni Vite-gyorsítótár-törlésből eredő, valódi `useRef is not defined` összeomlást is találtam és elhárítottam (2 eltérő react-dom verzió töltődött be egyszerre) — a szerver teljes újraindítása és egy friss böngésző-lap megoldotta, ez nem alkalmazáskód-hiba volt. Konzol-hiba nem jelentkezett, build hibamentes.

## 2026.09.16. — GYT-oldali eredménylap: a kitöltés dátuma és a nyomtatás gomb azonos magasságban

Marci jelezte: a GYT fiókban az eredménylap fejlécében a kitöltés dátuma és a nyomtatás gomb nincs azonos magasságban. Az ok: a dátum a névvel közös dobozban van, felülre igazítva, a gomb viszont a fejléc-sor MÁSIK eleme, ami alapból a teljes (kétsoros név-blokk magasságú) sorhoz igazítja középre — emiatt lejjebb csúszott. A gombot (kizárólag asztali nézetben, hogy a mobil elrendezést ne érintse) a sor tetejéhez igazítottam, ugyanahhoz a referenciaponthoz, amit a dátum-sor már eddig is használt.

**Tesztelve böngészőben:** DOM-méréssel megerősítettem, hogy a javítás után a dátum és a gomb teteje pontosan egybeesik (korábban 12px eltérés volt). Világos és sötét módban screenshot-tal is megerősítve, mobil nézetben ellenőrizve, hogy nincs regresszió (a gomb ott változatlanul a név mellett középre igazítva jelenik meg). Konzol-hiba nem jelentkezett, build hibamentes.

## 2026.09.16. — 2. fázis lezárása: alapos utóvizsgálat, hibajavítás és takarítás

Marci lezártnak tekintette a 2. fázist (állapotfelmérő), és kért egy alapos átvizsgálást: maradtak-e nem rögzített viszonyok, felesleges elemek, hibák. Átnéztem a teljes kérdőívet, a Contextet, az eredménylapot, a GYT-oldali nézetet, a gerincterhelés kalkulátort, a segédfüggvényeket, a teljes CSS-t, minden hivatkozott ikon/kép fájlt, és kerestem elhagyott debug-kódot/TODO-kat.

Egy valós, érdemi hibát találtam: a gerincterhelés kalkulátor a 9. lapon kívül sosem renderelődik, és minden állapotát nyers DOM-on tartja React state nélkül — emiatt, ha a kitöltő "előző"-vel elhagyta a kalkulátort (miután kitöltötte mind a 24 órát), majd "következő"-vel visszatért, a komponens újra mountolódott, és minden csúszka nullázódott, a 24 órát elölről kellett volna kitölteni. Javítottam: a komponens egy új `initialValues` propot kap, amivel a korábban már kiszámolt eredményből (amit amúgy is eltárol a Context) vissza tudja állítani a csúszkákat mountoláskor. Külön figyeltem arra, hogy ez az új prop ne törje meg a komponens meglévő `React.memo`-védelmét (ami egy korábbi hibát előz meg: ha a szülő minden csúszka-mozdulatkor újrarenderelne, a kalkulátor DOM-ja nullázódna) — a propot a lépésszámhoz kötöttem, nem a folyamatosan változó adatokhoz.

Emellett találtam és töröltem egy árva, sehol nem hivatkozott CSS-szabályt (`.eredmeny-dial-row`, egy korábbi elrendezés maradéka). Minden mást átvizsgáltam és rendben találtam — beleértve egy szándékosan még be nem kötött, jövőbeli fázisra előkészített modult (`deriveLimitations`), ami nem hiba, hanem Marci korábbi döntése szerint később kerül majd bekötésre.

**Tesztelve böngészőben:** a kalkulátor-hibát előbb reprodukáltam (24 óra → "előző" → "következő" → 0 óra), majd a javítás után megerősítettem, hogy a teljes kitöltés (össz-óraszám ÉS a részletes gerincterhelés/aktivitás pontszámok is) helyesen visszaáll. Ellenőriztem, hogy a további szerkesztés (egy csúszka utólagos módosítása) és egy teljesen új, üres kitöltés is helyesen működik továbbra is — nincs regresszió. Egy teljes, végponttól-végpontig menő tesztet is lefuttattam (kitöltés → beküldés → eredménylap). Konzol-hiba nem jelentkezett, build és típusellenőrzés hibamentes.

## 2026.09.16. — 1. fázis újranyitása: Calendly-válaszok a sales hívásoknál, "új" jelzés, "Pozitív elbírálás" email

Marci visszatért az 1. fázishoz: "amikor érkezik egy foglalás a Calendlytől, akkor a Calendly-ben megadott válaszokat látnunk kell az időpontra kattintva. Új sales foglalásokat ugyanúgy jelöljük, mint a gyt fiók új ügyfél-jelzését. A kiküldendő emailek között legyen olyan lehetőség is, hogy 'Pozitív elbírálás' — ezt lekattintva tűnik el a lime 'új' jelző az üf neve mellől." Mielőtt hozzákezdtem, 3 tisztázó kérdést tettem fel (mind az ajánlott, alapértelmezett választ kapta): a "Pozitív elbírálás" egy harmadik, szerkeszthető email-sablon legyen (a 2 elutasító sablon mellé, egy közös "email küldése" szekcióban mindhárom egyszerre látszik, nem a törlés gomb mögé rejtve); a Calendly-válaszok a MEGLÉVŐ hívás-részletek popupban jelenjenek meg (nem külön popupban); a demó Calendly-kérdéseket magam válasszam ki.

Minden sales-hívás (`SalesCall`) kapott egy `answers` (Calendly kérdés-válasz párok) és egy `isNew` mezőt. A hívás-részletek popup (korábban: piros/sárga/zöld gomb — törlés/nem jött/rendben) átalakult: a korábbi, trash-ikon MÖGÉ rejtett elutasító-sablon-választás megszűnt, helyette egy mindig látható "email küldése" szekció mutatja mind a 3 lehetőséget (1 zöld "Pozitív elbírálás" + 2 piros elutasító) egyszerre — az elutasítók megtartották a saját (könnyű) megerősítő lépésüket, mert törlő műveletek, a pozitív viszont azonnal hat, mert nem az. A "Pozitív elbírálás" saját, szerkeszthető sablonja az Üzenetek oldalon egy külön, lime szegélyű kártyában szerkeszthető, a 2 elutasító sablontól elválasztva. Az időpontra (nem csak a fogaskerékre) kattintva is megnyílik a popup. A "hívásaim" lista az új jelzést a GYT "ügyfeleim" oldalával TELJESEN AZONOS módon mutatja: lime "új" pötty a név mellett, ÉS ugyanaz a szám-jelvény az oldalsáv menüpontján — ehhez a sales adatokat tartó Context-et (korábban csak a /sales/* útvonalak alatt élt) a többi állapot-providerrel egy szintre kellett emelni, mert a menüsáv a route-oknál egy szinttel FELJEBB renderelődik.

**Tesztelve böngészőben:** a hívás-részletek popup megnyitva megerősítettem, hogy a 3 Calendly-kérdés/válasz helyesen megjelenik. "Pozitív elbírálás" gombra kattintva a lime "új" jelvény eltűnt a hívás mellől, ÉS az oldalsáv-pötty száma 2-ről 1-re csökkent. Egy elutasító sablonra kattintva a megfelelő nevű megerősítő kérdés jelent meg, "mégse"-vel helyesen visszalépett. Ugyanezt a teljes láncot (naptár-nézetben egy foglalt sávra kattintva → előnézet → fogaskerék → részletek) is ellenőriztem. Mobil nézetben (375×812) a badge és az időpont-gomb is helyesen működött. Világos és sötét módban is megerősítve screenshot-tal az Üzenetek oldal új kártyáját. A GYT saját "ügyfeleim" jelzése (a Context-hierarchia átalakítása után) változatlanul helyesen működik. Konzol-hiba nem jelentkezett (egy közbenső, hamis riasztást — a fejlesztői szerver korábban már dokumentált, munkamenet közbeni instabilitása — egy tiszta újraindítás után nem lehetett reprodukálni). Build és típusellenőrzés hibamentes.

## 2026.09.16. — "Pozitív elbírálás" email: pontos tárgy+szöveg, dátum/idő-jelölőkkel
...
## 2026.09.17. — "Pozitív elbírálás": landscape megerősítő popup, letiltott gomb küldés után, lime számláló ellenőrzése

Marci 3 kérést fogalmazott meg: a "Pozitív elbírálás" gombra kattintva megnyíló megerősítő popup is legyen landscape; küldés után a gomb ne legyen kattintható, "visszaigazolás kiküldve" felirattal; és ha nincs több nyitott (el nem bírált, nem lemondott) hívás, tűnjön el a lime számláló a "hívásaim" mellől.

A megerősítő popup megkapta a korábban bevezetett szélesített osztályt. A hívásokhoz új, `isNew`-tól külön `positiveSent` mező került — azért külön, mert egy már korábban hozzárendelt hívás (a demó Szűcs Viktória rekordja) is számíthat "elbíráltnak", anélkül hogy valaha `isNew` lett volna. A gomb ez alapján tiltódik le és vált feliratot. Mivel a küldés után a popupnak nyitva kell maradnia (hogy a gomb megváltozott állapota látszódjon), a megjelenített hívást mostantól mindig frissen, az aktuális listából olvassuk ki — a korábbi megoldás egy befagyott pillanatképet tárolt volna, ami nem tükrözte volna az új állapotot. A lime számláló eltűnése valójában már korábban is helyesen működött (a törölt hívások eleve kikerülnek a listából) — ez a kör csak megerősítette böngészős teszttel, hogy az új mező bevezetése után is.

**Tesztelve böngészőben:** megerősítettem, hogy a megerősítő popup szélesebb. Küldés után a popup nyitva maradt, a gomb letiltva, felirata megváltozott, és az oldalsáv-pötty élőben csökkent, majd egy második hívás elbírálása után teljesen eltűnt. A már eleve elbíráltként induló demó-rekordnál a gomb első megnyitásra is helyesen letiltva jelent meg, mobil nézetben is. Konzol-hiba nem jelentkezett, build hibamentes.

## 2026.09.17. — hiba: a "hívásaim" lime pöttye nem csökkent a "hozzárendelések" oldalon keresztüli hozzárendelésnél

Marci jelezte, hogy a "hívásaim" melletti lime pötty nem tűnik el, ha már nincs új hívás. Újra leteszteltem az előző körben már ellenőrzött utat (a hívás-részletek popup gombjai) — az továbbra is helyesen működött. A gyökér-ok egy másik, eddig nem tesztelt úton bukkant elő: a "hozzárendelések" oldal "adatok importálása" funkciója — ami az oldal saját tájékoztatása szerint is a fő, dokumentált módja a hívás ügyféllé alakításának — frissítette a hívás állapotát, de nem törölte az "új" jelzőt. Ha valaki egy friss hívást sosem nyitott meg a "Pozitív elbírálás" gombbal, hanem egyenesen hozzárendelte egy GYT-hez, a hívás örökre "új" maradt volna, a pötty sosem csökkent volna.

**Tesztelve böngészőben:** friss munkamenetben a "hozzárendelések" oldalon importálva és hozzárendelve egy "új" jelzős hívást, megerősítettem, hogy a "hívásaim" pötty azonnal helyesen csökkent. Konzol-hiba nem jelentkezett, build hibamentes.

## 2026.09.17. — a valódi ok: egy demó-hívás öt nappal későbbre volt ütemezve, ezért sosem látszott a "mai hívások" listában

Marci jelezte, hogy a lime pötty akkor is "1"-en ragad, ha már mindenkinek kiküldte a pozitív elbírálást. Újra végigjártam az előző körben javított utakat — mindkettő helyesen működött. Izoláltan tesztelve kiderült: pontosan "1" marad, ha valaki csak a "mai hívások" listán elérhető hívásokat bírálja el. A gyökér-ok nem logikai hiba volt, hanem egy könnyen észrevétlen demó-adat: az egyik "új" jelzős demó-hívás öt nappal a mai nap után volt ütemezve, ezért sosem jelent meg a fő, "mai hívások" nézetben — csak a naptárban, több hetet előre lapozva volt megtalálható. Áthelyeztem a másik két mai demó-hívással azonos napra, hogy minden el nem bírált tétel elérhető legyen a fő nézetből.

**Tesztelve böngészőben:** friss munkamenetben megerősítettem, hogy mindhárom demó-hívás megjelenik a "mai hívások" listában, és a listáról, a naptár megnyitása nélkül mindkét "új" hívást elbírálva a pötty teljesen eltűnt. Konzol-hiba nem jelentkezett, build hibamentes.

## 2026.09.17. — saját, kézzel felvett hívásnál a 2 elutasító-sablon helyett sima törlés
...
## 2026.09.17. — admin "anyagok kezelése": videó feltöltése a videókiosztáshoz, egyedi videó létrehozása

Marci kérésére egy új admin oldalsávelem ("anyagok kezelése") készült, amivel a 20 meglévő, torna-szintekhez tartozó kódhoz (S01–S13, A01–A07) videó tölthető fel, és emellett teljesen új, egyedi kódú/című videó is létrehozható, ami — a kérésnek megfelelően — nem kerül be az automata videókiosztás ajánlásba, csak a legördülő menübe.

Ez volt a projekt eddigi legnagyobb, valódi médiakezelést bevezető köre: eddig sehol nem volt fájl-feltöltési minta, és minden "videó" a kódbázisban egy statikus "▶" placeholder volt. Létrehoztam egy új, megosztott `MaterialsContext`-et (a `BlogContext` mintáját követve), ami a feltöltött fájlokat `URL.createObjectURL`-lel teszi ténylegesen lejátszhatóvá — ez az első hely a projektben, ahol valódi videó fut le, nem csak helykitöltő. Mivel nincs backend, ez munkamenet-szintű: oldal-frissítésre elvész, ugyanúgy, mint a projekt minden más demó-állapota.

A kulcs-döntés az volt, hogy a meglévő kódok (`ExerciseCode`) TypeScript-szinten zárt uniót alkotnak — egy admin által kitalált egyedi kód strukturálisan nem kerülhet be az automata ajánlási logikába (`SEQUENCES`/`suggestedSequence`), tehát a kérés ("ez nem kerül be az automata videókiosztás ajánlásba") extra szűrőlogika nélkül, magától teljesül. Egy `isBuiltInCode` segédfüggvény dönti el, melyik kód melyik kategóriába tartozik, ez különbözteti meg az admin oldal két szekcióját (meglévő kódok feltöltése / új, egyedi videó létrehozása, kód-egyediség ellenőrzéssel).

A GYT videókiosztás oldalán a kódok legördülője mostantól a feltöltött egyedi videókat is tartalmazza, és egy már kiosztott, videóval rendelkező szintnél egy "▶ videó" gomb nyit meg egy tényleges lejátszót — így a GYT látja, pontosan mit oszt ki. Az ÜF "szintjeid" oldalán, ha az adott szinthez van feltöltött anyag, a korábbi placeholder helyett a valódi videó jelenik meg.

**Tesztelve böngészőben:** adminként feltöltöttem egy videót S01-hez (a natív fájl-inputot egy `Object.defineProperty`+`change`-esemény technikával szimulálva, mert a böngésző-automatizálás natív fájlválasztót nem tud kezelni) — a sor "feltöltve"-re váltott, a videó ténylegesen lejátszható volt. Létrehoztam egy "X01" egyedi videót is: a kód-egyediség ellenőrzés helyesen hibát adott S01-re próbálva; a GYT legördülőjében megjelent mind a 20 beépített, mind az 1 egyedi kód (a lista végén); kiosztás után a "▶ videó" gomb ténylegesen megnyitotta az előnézetet. Az ÜF oldalon (a teljes állapotfelmérő kitöltése után) az S01-videó valódi, lejátszható videóként jelent meg. Világos és sötét módban, valamint mobil nézetben (375×812) is megerősítve, konzol-hiba nem jelentkezett, `npm run build`/`tsc -b` hibamentesek.

## 2026.09.17–18. — "3. fázis" újraindítása: folyamat-hiba, revert, majd újraegyeztetett terv

Marci 2026.09.17-i kérésére ("oktatóanyag+ tudáspróba + munkafüzet létrehozása az üf fiókban és munkafüzet hozzáférés a gyt számára") azonnal, tisztázó kérdés nélkül végigépítettem és commitoltam egy első verziót. Marci jelezte, hogy ez hiba volt — kérdezés nélkül kezdtem dolgozni. A teljes kört egy `git revert`-tel visszavontam és felpusholtam, a repó visszaállt a 150. pont utáni állapotra.

Ezután 2 körben tisztázó kérdéseket tettem fel, mielőtt bármit építettem volna: a tudáscheck egy ideiglenes "teljesítve" gombbal induljon (valódi kérdések helyett, amíg meg nem érkeznek), a munkafüzet valódi, kitölthető mezőkkel készüljön (GYT csak olvassa), az oktatóanyag tartalmát Marci adja meg (nem én találom ki). Marci pontosította: 2 lecke ("A gerinc biomechanikája", "Gerinchasználat a gyakorlatban"), leckénként kb. 8-10 fejezet, és az admin tudjon leckéket/fejezeteket felvenni (placeholder UI, videó nélkül). További kérdésekre: a munkafüzet-táblázat sorai dinamikusan bővíthetők, a tudáscheck ideiglenes gombbal induljon, az admin-felület a meglévő "anyagok kezelése" oldalhoz kerüljön. Marci feltöltötte a `Munkafüzet.odt` mintát — ez lett a munkafüzet tartalmának egyetlen hiteles forrása (2 feladat, mindkettő egy 2-oszlopos táblázattal).
...
Egy harmadik kör finomítást is kért még aznap: a 2 oszlop legyen pontosan egyenlő szélességű, és a nyíl ne csak a fejlécben, hanem soronként is jelenjen meg a 2 hasáb között. Egy keskeny, középre igazított nyíl-oszlopot vezettem be a 2 (mostantól `colgroup`+`table-layout:fixed` miatt garantáltan egyenlő szélességű) tartalmi oszlop közé — a nyíl minden sorban ott áll. Böngészőben megerősítve, asztali nézetben, világos és sötét módban, mindkét (szerkeszthető és csak-olvasható) nézetben.

Negyedik kör: Marci jelezte, hogy mobilon sötét módban a beviteli mezők kontrasztja gyenge. Valós hibát találtam: a mező szövegszíne sosem volt felülírva, a Bootstrap majdnem-fekete alapértelmezése érvényesült a saját sötétített háttér ellen. Javítottam: a háttér-tintézés erősödött, a szöveg pedig a téma szövegszínét kapja explicit módon. Böngészőben megerősítve mobilon, sötét és világos módban is, ténylegesen beírt szöveggel.

Ugyanaznap Marci egyszerre 4 további hibát jelzett: a beviteli mezők sötét módban még mindig túl sötétek, a mobilos kitöltő szöveg feketének tűnt, az asztali nyilak feketék voltak, és fókuszáláskor a piros mező kerete tévesen zöldre váltott. Mindegyikre valós okot találtam: a háttér-tintézés alapja egy világosabb tokenre váltott (erősebb, világosabb doboz), a szöveghez `-webkit-text-fill-color` és egy hiányzó `color-scheme` deklaráció is bekerült (Safari/WebKit natív sötét-mód-automatizmusa ellen), a nyíl-oszlop színéről lemaradt a korábban máshol már alkalmazott Bootstrap-felülíró `!important`, a fókusz-keret pedig egy globális, minden formra vonatkozó márkaszín-szabállyal ütközött, amit most saját, piros/zöld fókusz-szabály vált ki. `getComputedStyle`-lal (nem csak vizuálisan) ellenőrizve minden pontot: a tényleges színkódok pontosan a várt tokeneket adták vissza mindkét témában.
...
## 2026.09.19–21. — GYT ügyfélválasztás: tiszta állapot belépéskor + nav-szintű automatikus kapu (ismételt folyamat-hiba után)

Marci visszajelzett, hogy a fenti javítás után rögtön Péter adatait látta bejelentkezés után, kérdés nélkül elkezdtem kalapálni ahelyett, hogy megbeszéltük volna — ugyanaz a hiba, mint korábban a 3. fázis legelején. Több körben tisztáztuk a valós okot (a kiválasztott ügyfél a böngészőben végleg megmaradt, amíg valaki kifejezetten ki nem jelentkezett — egy sima belépés ezt nem törölte), majd Marci pontosan megfogalmazta az elvárt viselkedést, amit szó szerint visszaírtam neki, és csak a kifejezett jóváhagyása után kezdtem kódolni.

A belépés mostantól minden alkalommal törli a mentett ügyfél-kiválasztást. A 3 érintett GYT-oldal (videókiosztás, állapotfelmérők, munkafüzet) eddig egyenként, saját belső üzenettel kezelte az "nincs kiválasztott ügyfél" esetet — ez most egy közös, nav-szintű route-kapura került (az állapotfelmérő-kapu mintájára): ha nincs kiválasztás, automatikusan az "ügyfeleim" oldalra irányít, onnan a választás után vissza az eredeti menüpontra. A "naptár" kimarad ebből, mert nem ügyfél-specifikus.

**Tesztelve böngészőben:** valódi bejelentkezéssel, előzetesen beállított "korábbi" kiválasztással — belépés után a kiválasztás törölve volt. Mindhárom érintett menüpont ügyfél nélkül azonnal, automatikusan átirányított, választás után pontosan vissza a megfelelő oldalra. A naptár változatlanul elérhető maradt ügyfél nélkül is, a kiválasztás menüpontváltás közben megmaradt. Mobil nézetben is megerősítve, konzol-hiba nem jelentkezett, `npm run build`/`tsc -b` hibamentesek.

## 2026.09.21. — "ügyfeleim" oldal: "Szűrés" legördülő
...
## 2026.09.21. — "4. fázis" indítása: GYT-oldali dokumentáció

Marci elindította a "4. fázisnak" nevezett kört: a GYT fiókban, ügyfélhez rendelt, szöveges dokumentáció, gyorsgombokkal. Kódírás előtt 2 körben tisztázó kérdésekkel egyeztettünk (AskUserQuestion), a korábbi, kérdezés nélkül elkezdett munka miatti hiba tanulsága szerint.

Megépült a `DokumentacioContext` (ügyfelenként 6, mindig elérhető "alkalom", a naptártól függetlenül), a `GytDokumentacio` oldal (`/gyt/dokumentacio`, a `GytClientGate` alatt), a munkafüzet-mintájú mentés/szerkesztés UX egy új réteggel: 3 hetes szerkesztési ablak az első rögzítéstől számítva, a 6. (záró) alkalom rögzítése pedig az ÖSSZES alkalmat azonnal, véglegesen lezárja (archiválás), a 3 hetes ablaktól függetlenül. Az 1. alkalomnál a meglévő Állapotfelmérő eredménylap (`Eredmenyeim`) ténylegesen beágyazva jelenik meg. A GYT saját, közös gyorsgomb-listát vehet fel/törölhet, kattintásra a szöveg a mező végéhez fűződik. A "PDF-export" a bevált `window.print()` mintát követi, egy új `.dokumentacio-print-only` CSS-osztállyal (`components.css`), ami nyomtatáskor az összes alkalmat + az eredménylapot egymás után jeleníti meg, míg képernyőn csak a kiválasztott alkalom látszik. A GYT nav-menü korábban zárolt "dokumentáció" pontja feloldva.

**Tesztelve böngészőben:** gyorsgomb felvétele+beszúrása, rögzítés utáni zárolt/szerkesztés-váltás, alkalmak közötti elkülönítés (egy `textContent`-alapú hamis riasztást `innerText`-tel tisztáztunk — a mindig-DOM-ban-lévő nyomtatási blokk okozta), a 3 hetes szerkeszthetőségi határidő dátuma, a záró alkalom rögzítése utáni azonnali, teljes archiválás (a korábban még szerkeszthető 1. alkalom is lezárult), a nyomtatási blokk tartalma, a GYT-kapu (ügyfélválasztásra irányítás + visszatérés) valódi route-váltással megerősítve. Világos, sötét és mobil (375×812) nézetben is rendben. Konzol-hiba nem jelentkezett, `npm run build`/`tsc -b` hibamentesek.

Marci utólag kérte, hogy a gyorsgomb-szekció a szöveges mező ALÁ kerüljön (eddig fölötte volt) — átrendezve, ellenőrizve.

Majd egy újabb kiegészítést kért: "az első alkalom dokumentációja eltér a többitől, itt az állapotfelmérés folytatódik" — Történet, Tünetek, Rizikó, Inspekció (gerinc görbületek, fej helyzet), Mozgásvizsgálat (fej emelés, fej lehajtás, kar eleváció, gerinc flexió, gerinc extenzió), Tesztek (sarokemelés jobb-bal) előre címkézett, kezdetben üres, szerkeszthető mezőkkel. 4 tisztázó kérdés után épült meg: ez a struktúra az általános szöveges mező MELLÉ kerül (nem helyette), üresen indul (nincs sablon-szöveg), a sarokemelés egy közös mező, és az egész alkalom egyetlen közös "Rögzítés" gombbal mentődik. A `DokumentacioContext` kapott egy `ASSESSMENT_SECTIONS` struktúra-leírót és az entry egy `assessment` mezőt (csak az 1. alkalomnál), a `GytDokumentacio` pedig egy újrafelhasználható `AssessmentSection` komponenst (szerkeszthető és csak-olvasható/nyomtatási módban is).

**Tesztelve böngészőben:** csak strukturált mezőket kitöltve (általános szöveg üresen hagyva) a Rögzítés sikeresen mentett, szerkesztésre visszaváltva a korábbi értékek helyesen visszatöltődtek, a nyomtatási blokk tartalmazta az állapotfelmérés-folytatás szakaszt a mentett adatokkal. Világos, sötét és mobil nézetben is rendben, `npm run build`/`tsc -b` hibamentesek.

Marci felvetette: "jó lenne, ha a gyt tudná személyre szabni, és ő tudna hozzáadni beviteli mezőket" — a beállítós verziót javasoltam (közös sablon egy külön oldalon, nem ügyfelenkénti helyi szerkesztés), amit elfogadott, azzal a kiegészítéssel, hogy a MÁR MEGADOTT mezők is szerkeszthetők legyenek, és a módosítás NE legyen visszamenőleges (egy már rögzített dokumentáció a saját rögzítéskori mező-szerkezetét őrizze meg örökre). 4 tisztázó kérdés után épült meg: `DokumentacioContext` fix `ASSESSMENT_SECTIONS` konstansa helyett egy szerkeszthető `assessmentTemplate` állapot lett (CRUD-függvényekkel), az entry pedig a saját, rögzítéskor lefényképezett szerkezetét (`assessmentSnapshot`) tárolja az értékekkel együtt. Új oldal: `/gyt/dokumentacio-beallitasok`, saját nav-menüponttal, ahol a GYT szakaszokat/mezőket vehet fel, nevezhet át, törölhet.

**Tesztelve böngészőben:** egy szakaszt átnevezve + egy mezőt hozzáadva, egy soha nem rögzített ügyfélnél azonnal megjelent az új szerkezet; egy másik ügyfélnél az 1. alkalmat előbb rögzítve, majd a sablont tovább módosítva (mező törölve, szakasz újra átnevezve) a MÁR RÖGZÍTETT dokumentáció helyesen megőrizte a régi szerkezetét és a mentett szöveget, míg egy harmadik, soha nem rögzített ügyfélnél már a legfrissebb sablon látszott — a "nem visszamenőleges" szabály helyesen működik. Világos, sötét és mobil nézetben is rendben, `npm run build`/`tsc -b` hibamentesek.
...
Marci: "A szerkesztőnél legyen olyan gomb az alján, hogy 'módosítások mentése' és egy olyan, hogy mégse..." — eddig a beállítás-oldal minden szerkesztése azonnal, billentyűleütésenként élesen mentődött, visszavonás nélkül. 3 tisztázó kérdés után (mind "Ajánlott" válasszal: az egész oldalra vonatkozó egy gombpár, hozzáadás/törlés is a ciklus része, az előnézet a gépelt piszkozatot mutatja) átépült a beállítás-oldal: saját, helyi piszkozat-állapotot szerkeszt, ami csak "módosítások mentése"-re kerül át a közös sablonba (`replaceAssessmentTemplate`), "mégse"-re pedig eldobódik. Mentés után a szerkesztő mezők csak-olvasható listára váltanak, "✓ mentve" jelvénnyel és "szerkesztés" gombbal — ugyanaz a minta, mint a munkafüzet/dokumentáció "Rögzítés" ciklusa. A context 6 granuláris CRUD-függvénye helyett egyetlen `replaceAssessmentTemplate` maradt.

**Tesztelve böngészőben:** átnevezés + mentés helyesen frissítette a közös sablont (egy másik ügyfélnél is megjelent); egy próba-átnevezés "mégse"-vel helyesen visszaállt a korábban ténylegesen mentett állapotra. Világos, sötét és mobil nézetben is rendben, `npm run build`/`tsc -b` hibamentesek.

Marci: "A 6 dokumentációs alkalom mellett, amikor már a 6. is rögzítve lett, akkor kerüljön oda egy új gomb: további jegyzetek..." — 3 tisztázó kérdés után (mind "Ajánlott": nincs zárolás/időkorlát rajta, egyetlen bővíthető mező, a pirula-sorba illeszkedő 7. elem) megépült egy új, ügyfelenkénti, MINDIG szabadon szerkeszthető jegyzet-mező, ami csak a 6. (záró) alkalom rögzítése (archiválás) után válik elérhetővé egy új "további jegyzetek" pirulaként. Nincs külön "mentés" lépés, a szöveg azonnal a context-be kerül.

**Tesztelve böngészőben:** a pirula csak archiválás után jelent meg; a mező szövege alkalmak közti váltás után is megmaradt, a nyomtatási blokk is tartalmazta. Világos, sötét és mobil nézetben is rendben, `npm run build`/`tsc -b` hibamentesek.

Marci két kiegészítést kért: (1) "a kapcsolók háttere váltson át szögletesebb módra, ha több sorba rendeződnek az alkalmak dokumentációi" — a pirula-sor mostantól `ResizeObserver`-rel VALÓDI tördelést érzékel (nem csak becsült szélesség-töréspontot), és ha ténylegesen több sorba törik, a teljesen kerek kapszula-háttér egy kevésbé lekerekített, "szögletesebb" háttérre vált. (2) "A további jegyzeteken is legyen mentés, nehogy véletlenül törlődjön egy korábban beírt adat" — a jegyzet-mező eddigi, minden billentyűleütésre azonnal mentő viselkedése helyett mostantól saját piszkozatot tart, ami csak "mentés" gombra kerül át a tartós tárolóba (de a mező maga továbbra sem zárolódik, mindig gépelhető marad).

**Tesztelve böngészőben:** a 6 alkalom-pirula egy sorban kerek maradt, a 7. ("további jegyzetek") pirula megjelenésével kétsoros tördelésre váltva a háttér azonnal szögletesebbre változott. A jegyzet-mezőnél mentés nélkül másik fülre váltva a beírt piszkozat elveszett (véletlen felülírás helyett), ténylegesen elmentve viszont megmaradt. Világos, sötét és mobil nézetben is rendben, `npm run build`/`tsc -b` hibamentesek.
...
## 2026.09.23. — 5 vizuális/UX hiba javítása (Marci saját tesztelése)

Marci a gépi átvizsgálás után saját maga is végigtesztelte a dokumentáció fület, és 5 hibát talált: (1) az alkalmak doboz, az eredménylap doboz és a "lezárva" doboz sarok-lekerekítése nem volt egységes — élő méréssel kiderült, hogy csak a nemrég bevezetett, több sorba tördelt pirula-háttér tért el (8px a többi 16px-es dobozhoz képest), ez lett kiigazítva; (2) mobilon a pirula-gombok balra tömörödve, egyenlőtlenül jelentek meg — most egy dedikált 2-oszlopos rács rendezi egyenletesen; (3) a beágyazott eredménylap Név-sora mobilon görgetéskor "beragadt" (egy MÁSODIK, felesleges sticky fejléc volt rajta, az önálló "állapotfelmérők" oldalról örökölve) — az `Eredmenyeim` komponens kapott egy `embedded` prop-ot, ami ezt kikapcsolja beágyazott használat esetén, az önálló oldalon változatlan maradt; (4) a "beállítások"/"nyomtatás" gombok a lap fejlécéből a lap aljára, a mentés/rögzítés rész alá kerültek.

**Tesztelve böngészőben:** mindhárom doboz/sáv rádiusza 16px-nek mérve; mobilon szabályos 2 oszlopos pirula-elrendezés; görgetéskor a Név-sor a tartalommal együtt gördül, csak az oldal saját fejléce marad rögzítve; az önálló "állapotfelmérők" oldal saját sticky fejléce változatlanul működik; a gombok a lap alján jelennek meg. Világos, sötét és tablet nézetben is rendben, `npm run build`/`tsc -b` hibamentesek.

Marci: "Rögzítsünk egy szabályt a dokumentációhoz: csak akkor lehet a következő dokumentációt szerkeszteni, ha az előző már ki lett töltve, mentve lett." Egyértelmű szabály, közvetlenül megvalósítva: a 2. alkalom csak az 1. mentése után nyílik meg, a 3. csak a 2. után, és így tovább — az 1. alkalomnak nincs előzménye. Mivel a 6. (záró) alkalom is ezen a szabályon megy keresztül, ez természetesen kizárja, hogy a GYT sorrenden kívül, egyenesen a záró alkalomig ugorjon. Útközben egy apró, korábban észrevétlen hiányosságot is pótoltam: egy sose mentett, de zárolt alkalomnál eddig nem jelent meg magyarázat, most már igen.
...
## 2026.09.24. — 9 további checklist-finomítás: megtartás-idő felülbírálása, szint-sáv, csúszka-stílusok, egyesített diagramok, előző szint, edzésenkénti tünet

Marci egy 9 pontos listát küldött a checklist további finomítására. A legtöbb alacsony kétértelműségű volt (szöveg-csere, elrendezés, meglévő csúszka-stílusok átvétele, egy szimmetrikus "előző szint" gomb), de az utolsó — hogy egy extra edzés hozzáadásakor is jelölhető legyen, volt-e közben tünet, és ez a halmozott oszlop-diagramon is megjelenjen — valódi adatmodell-döntést igényelt. Két tisztázó kérdés után (a teljes 7 opciós tünet-legördülő az extra edzésnél is, azonnali inline megerősítéssel) neki lehetett esni a kódolásnak.

Megépült: a megtartás-idő (mostantól "mai javasolt megtartási idő") a fejlécben kattintható, egy legördülőben 1-től a napi javasolt értékig szabadon csökkenthető (önálló, (dátum,szint) kulcsú `holdOverrides` térképben tárolva, függetlenül a napi űrlap mentésétől); a szint neve egy külön, szürke sávba került a kártya tetején, ide kerültek a szint-váltó gombok is (az új "előző szint"-tel együtt, ami a KORÁBBI szint eredeti kezdő dátumát állítja vissza, nem indítja újra a progressziót); az "edzés megvolt" és "tünet gyakorlat közben" egymás mellé kerültek; a "tünet napi intenzitása" csúszka átvette az állapotfelmérő dinamikus szín-interpolációját (ezt a logikát egy közös `src/utils/intensityColor.ts` modulba emeltem ki, hogy mindkét hely ugyanabból éljen); a "terhelés optimalizálás" csúszka a gerincterhelés kalkulátor teal→mint gradiensét vette át; a 3 diagram egy közös, elválasztókkal tagolt dobozba került, közös Recharts `syncId`-vel, ami beépítetten szinkronizálja a 3 diagram kurzorát/tooltipjét egymással; az edzésnapok-diagram tooltipje mostantól az aznapi megtartás-időt is kiírja. A legnagyobb változás: egy nap "edzései" mostantól egy `workouts` tömb (a korábbi puszta számláló helyett), mindegyik saját tünet-válasszal — "még egy edzést hozzáadok" egy inline legördülőt nyit, és az oszlopdiagram edzésenként (nem naponta) színezett, halmozott szegmensekkel rajzolja a napot.

**Tesztelve böngészőben:** valódi ÜF-bejelentkezéssel és állapotfelmérő-kitöltéssel az összes új elem (szint-sáv, kattintható megtartás-idő legördülő pontos 1-7 mp tartománnyal, egymás melletti mezők, mindkét új csúszka-stílus) helyesen jelent meg; "következő szint kezdése" + "előző szint" sorrendben tesztelve a korábbi szint adatai és megtartás-ideje sértetlenül visszatértek; az inline tünet-választó hozzáadás után "edzés megvolt (+1 további edzés)" jelent meg helyesen. GYT-oldalon a halmozott oszlopok a várt sorrendben, várt színekkel jelentek meg, a tooltip helyesen mutatta a megtartás-időt és az edzésenkénti tünet-listát, és a 3 diagram kurzora ténylegesen szinkronban mozgott. Világos, sötét és mobil nézetben is rendben, konzol-hiba nem jelentkezett, `npm run build`/`tsc -b` hibamentesek.

Marci két hibát jelzett vissza az előző kör után: a "még egy edzést hozzáadok" gombra kattintva eltűnt a "szerkesztés" gomb, és a 3 diagram kurzora nem egységes (az oszlopdiagram széles szürke sávja vs. a vonaldiagramok vékony vonala), ami azt a benyomást keltette, hogy a popup csak az épp hoverelt diagramnál jelenik meg. Az első a "szerkesztés" gomb és az edzés-hozzáadó widget közös sorának véletlen kitolódása volt — külön sorra kerültek. A második egy kézzel írt, mindhárom diagramon azonos, vékony szaggatott vonalú kurzor-komponenssel lett javítva (a Recharts alapértelmezett, diagramtípusonként eltérő kurzor-alakja helyett).

**Tesztelve böngészőben:** a "szerkesztés" gomb a helyén maradt az edzés-hozzáadás közben, a 3 diagram kurzora pixelre egyforma stílusú lett, és mindhárom tooltip egyszerre jelent meg hoverkor. Világos és sötét módban is rendben. Egy, a fejlesztői szerver újraindításához kötődő, egyszeri konzol-hiba jelentkezett tesztelés közben, de ismételt friss betöltésekkel nem reprodukálódott. `npm run build`/`tsc -b` hibamentesek.
...
Marci: "így bénán néz ki az oszlopdiagram. inkább egy kis szünettel kezdődjön az összes diagram, hogy az oszlop szépen kiférjen." Az előző javítás (pont-skála kényszerítése) az első/utolsó oszlop KÖZÉPPONTJÁT pontosan a rajzterület szélére állította, ezért az oszlop fele csonkán jelent meg. Mindhárom diagram X-tengelye egy közös, 14px-es paddinget kapott mindkét oldalon — ez egyformán befelé tolja az első/utolsó napi pozíciót mindhárom diagramon (az illesztés megmarad), az oszlopdiagram `<Bar>`-jai pedig egy fix 20px-es szélességet.

**Tesztelve böngészőben:** DOM-méréssel megerősítve, hogy a 3 diagram napi pozíciói a padding hozzáadása után is pixelre egyeznek, az oszlopok pedig most már teljes szélességükben, csonkolás nélkül látszanak. Világos, sötét és mobil nézetben is rendben, `npm run build`/`tsc -b` hibamentesek.

Marci: "gyt fiókban a gyt is láthassa a teljes időszakot, vagy egy kiválasztott szintet." Az ÜF-oldali checklisten már meglévő "ezen a szinten"/"teljes időszak" váltó átkerült a GYT-oldali nézetbe is (`GytChecklist.tsx`) — "kiválasztott szint" felirattal (mivel a GYT bármelyik szintet megnézheti a legördülőből, nem csak "a" jelenlegit). "Teljes időszak" nézetben a szint-választó eltűnik, a kártya címe "teljes időszak"-ra vált, és a diagramok minden szint adatát együtt, a bejegyzések természetes (nem 14 napra kényszerített) szélességében mutatják.

**Tesztelve böngészőben:** a váltás mindkét irányban helyesen működött — "teljes időszak"-ban a legördülő eltűnt és a 9 napos teljes adatsor jelent meg, "kiválasztott szint"-re visszaváltva a 14 napos idősáv és a szint-választó is helyreállt. Világos, sötét és mobil nézetben is rendben, `npm run build`/`tsc -b` hibamentesek.

Marci megkért, hogy UX szempontból vizsgáljuk át a napi checklist beviteli mezőt — ez az, amit az ügyfél fél éven át naponta tölt, tehát a sebesség/egyszerűség kritikus. Építés előtt szöveges átvizsgálást adtam: a mezők köre spec-kötött, de a kitöltés mechanizmusa javítható (túl sok blokk a mezők előtt, 51 lépéses csúszka mobilon nehezen találgatható, elvont "terhelés optimalizálás" mező, "Rögzítés" gomb messze a mezőktől). 3 kérdést tisztáztunk: marad az explicit Rögzítés gomb (de mobilon a képernyő aljához rögzítve); a tünet időtartama/intenzitása mezők NE tűnjenek el semmilyen esetben (ezek a nap egészének tünet-állapotára kérdeznek, nem csak a gyakorlat közbenire); a "tünet napi időtartama" csúszka helyett gyors gombsor (nincs/15 perc/30 perc/1 óra/néhány óra/egész nap/egyéb).

Megépült: `DurationPicker` komponens a gyors gombsorral + "egyéb" esetén a meglévő pontos csúszkával; a "Rögzítés" gomb mobilon (a projekt 767.98px-es töréspontja alatt) a képernyő aljához rögzítve, asztali nézetben változatlanul a normál helyén.
...
## 2026.09.24. — a checklist napi beviteli mező teljes újratervezése, nulláról

Marci: "A checklist eredményei kártya az rendben van, a logikák is rendben, de a beviteli mező nem jó. Újrakészítjük nulláról." Egy teljes, szó szerinti diktálást adott az új felületről (sötét fejléc a szint nevével és a lime megtartás-idővel, "edzés rögzítése" kör-ikon gombbal, "volt közben tünet?" kérdés, "Mai tünetek" szakasz sávos időtartam-legördülővel, két csúszka, "Mentés" gomb). 3 tisztázó kérdés után (a többszöri edzés-rögzítés marad, a szint-váltás a "⋯" menüben marad, a "nem hajolós nap" lesz a csúszka látható címe) nekiálltam az újraépítésnek.

A diagramok és a mögöttes logika változatlan maradt — csak a napi űrlap épült újra nulláról. A tünet-legördülő új értékkészletet kapott (nem/húzódás/görcs/fájdalom/nyilallás/izomláz/más), az időtartam a korábbi folytonos csúszka helyett egy sávos legördülő lett (5 perc – 12 óra+). Az "edzés rögzítése" gomb a projektben már meglévő zöld kör-ikon mintát vette át; többszöri megnyomásra minden edzés saját, azonnal módosítható tünet-legördülőt kap. Saját döntésként az űrlap mostantól mindig szerkeszthető (nincs külön "szerkesztés" mód) — a mai bejegyzés, ha van, előre kitöltve jelenik meg.

**Tesztelve böngészőben:** a teljes folyamatot valódi bejelentkezéssel és állapotfelmérő-kitöltéssel kipróbálva — az "edzés rögzítése" gomb, a többszöri edzés-hozzáadás saját legördülőkkel, a sávos időtartam-választás és a mentés utáni előre-kitöltött állapot mind helyesen működtek. A GYT-oldali diagramok az új tünet-értékkészlettel is helyesen jelenítik meg a halmozott oszlopokat és a tooltipeket. Világos, sötét és mobil nézetben is rendben, `npm run build`/`tsc -b` hibamentesek.

Marci két apró javítást kért az előző körre: az "edzés rögzítve" szöveg + külön kör-pipa gomb helyett egy EGYETLEN lime gomb ("edzés rögzítése", kattintásra "N mai edzés rögzítve" + pipa-ikon), és megkérte, keressek rá, volt-e már ilyen formájú gomb más fázisokban. Egy kutató subagent megtalálta: az állapotfelmérő "beküldés" gombja pontosan ezt a mintát használja (lime `.btn-fyb-highlight` + szöveg utáni pipa-ikon) — ezt vettem át. A "Mentés" gomb a korábbi, mobilon rögzített sáv helyett most egyszerűen a kártya saját alján, középre igazítva jelenik meg.

**Tesztelve böngészőben:** a lime gomb helyesen váltott "edzés rögzítése" → "1 mai edzés rögzítve ✓" → "2 mai edzés rögzítve" szövegek közt kattintásonként; a "Mentés" gomb mind asztali, mind mobil nézetben a kártyán belül, középen, nem rögzítve jelent meg. Világos és sötét módban is rendben, `npm run build`/`tsc -b` hibamentesek.

Marci négy további finomítást kért: "edzés rögzítése" → "edzés hozzáadása"; kattintás után az első gomb váljon egy statikus, letiltott "edzés hozzáadva" visszaigazolássá (a sales hívás-részletek "visszaigazolás kiküldve" gombjának mintájára), mellette egy külön, végig aktív "még egy edzés hozzáadása" gombbal; egy halvány, doboz-szélig érő elválasztó csík az időtartam/intenzitás és intenzitás/"nem hajolós nap" mezők között; a "Mentés" gomb kattintásra váltson "mentve"-re.

**Tesztelve böngészőben:** az első gomb kattintás után helyesen letiltva, "edzés hozzáadva" felirattal jelent meg, mellette az aktív "még egy edzés hozzáadása" gombbal; az elválasztó csíkok a kártya teljes szélességében futottak; a "Mentés" gomb kattintásra azonnal "mentve"-re váltott. Világos és sötét módban is rendben, `npm run build`/`tsc -b` hibamentesek.

Marci szólt, hogy a "mentve" gomb és a "✓ mai nap rögzítve" jelvény ugyanazt jelenti — kérte, hogy az utóbbit töröljük, a kinézetét pedig vigyük át az "edzés hozzáadva" gombra. A fejlécben megjelenő badge-fyb jelvény törölve, a stílusa (kis méret, kövér, nagybetűs, lime háttér/navy szöveg) egy új `.checklist-workout-done-badge` osztályba került, amit a letiltott "edzés hozzáadva" gomb kap meg a normál `btn-fyb-highlight` helyett — így az nem a szokásos fakított (`opacity: 0.5`) letiltott-gomb kinézetet kapja, hanem a badge-fyb élénk, teljes lime kinézetét, mivel egy MÁR megtörtént állapotot jelez. Útközben egy hibát is javítottam: első nekifutásra elfelejtettem átvinni a háttér-/szövegszínt az új osztályba, így a gomb szürkére fakult — a `background-color: var(--lime)`/`color: var(--navy)` pótlásával javítva.

**Tesztelve böngészőben:** valódi bejelentkezéssel és állapotfelmérő-kitöltéssel megerősítve, hogy a "✓ mai nap rögzítve" jelvény eltűnt, és az "edzés hozzáadva" gomb a badge-fyb stílusában (lime háttér, navy szöveg, nagybetűs felirat, arányosan kisebb pipa-ikon) jelenik meg, a "még egy edzés hozzáadása" gomb változatlanul a normál lime stílust viseli. A szín sötét módban is fixen lime/navy maradt. Mobil nézetben (375px) nem jelentkezett túlcsordulás. `npm run build`/`tsc -b` hibamentesek.

## 2026.09.25. — a checklist beviteli doboz görgetés nélküli mobil-elrendezése + 2 grafikon-javítás

Marci: "találj ki egy olyan elrendezést, amivel a checklist beviteli doboza elfér görgetés nélkül telefonon. Bármit megváltoztathatsz, csak a funkciók maradjanak." Élő méréssel indultam: a doboz mobilon (375×812) 697,9px magas volt, a fölötte lévő topsáv+cím miatt ez ~37px-cel túlnyúlt a képernyőn. A funkciók köre változatlan maradt, csak az elrendezés tömörödött: a fejléc két sora egybe vonva (a megtartás-idő számának betűmérete kicsit csökkent, de továbbra is nagyobb marad a környező szövegnél); a "Mai tünetek" önálló alcím törölve, a szerepét egy keskenyebb margójú elválasztó csík vette át; az "időtartam" mező a "címke fölötte / legördülő alatta" helyett egy sorba tömörült; a csúszkák (`CenteredSlider`) címe és nagy száma egy sorba került (a korábbi, középre igazított önálló sor helyett), a min-max felirat-sor pedig törölve. A kártya paddingja és a gombsorok/mezők közti margók is szűkebbek lettek.

Marci külön jelezte: "az eredmények grafikonján valamelyik oszlopnak le van kerekítve a sarka, valamelyiknek nincs. Egységesítsd, mind lekerekített sarokkal legyen." A hiba oka: az "edzésnapok" oszlopdiagramon a lekerekítés csak a LEGFELSŐ lehetséges edzés-sávnak volt beállítva, de kevesebb edzéses napokon ez a sáv meg sem jelenik, így azok szögletesek maradtak. Megoldás: egy saját `shape`-komponens naponta eldönti, melyik szegmens a ténylegesen látható legfelső, és csak azt rajzolja lekerekítve. Útközben belefutottam egy recharts-hibába (egyedi `shape` + animáció kombinálva a magasság mindig 0 maradt) — `isAnimationActive={false}` oldotta meg.

Harmadik kérés: "telefonos nézetben is egyszerre mozogjon a kurzor a 3 grafikonon." A recharts forráskódját átnézve a szinkron logika maga érintőn is működik, de a diagram-terület alapértelmezett `touch-action` beállítása miatt egy vízszintes húzás-gesztus könnyen natív oldal-görgetéssé alakulhat, mielőtt a recharts touchmove-figyelője megkapná — `touch-action: pan-y` a diagram-wrapperen ezt megelőzi (a függőleges görgetés marad, a vízszintes húzást a böngésző nem viszi el). Ezt a konkrét javítást valódi érintő-gesztussal nem lehetett tesztelni ebben a böngésző-eszközkészletben, a döntés forráskód-elemzésen és a `touch-action` szabványos viselkedésén alapul.

**Tesztelve böngészőben:** élő pixel-méréssel megerősítve, hogy a beviteli doboz mobilon (üres állapotban és 1 edzéssel is) teljesen látszik görgetés nélkül; minden gomb/mező/csúszka működése változatlan (Mentés→mentve, edzés hozzáadása→EDZÉS HOZZÁADVA, "⋯" menü, megtartás-idő szerkesztő). Az oszlop-lekerekítés DOM-vizsgálattal megerősítve minden nap típusnál helyes. Világos és sötét módban is rendben, asztali nézetben a fejléc egy sorban fut ki. `npm run build`/`tsc -b` hibamentesek.

## 2026.09.25. — checklist teljes újratervezés + a mobil kurzor-szinkron végleges javítása

Marci jelezte, hogy az előző kör `touch-action: pan-y` javítása a telefonján mégsem oldotta meg a kurzor-szinkron problémát. Mivel a jelenlegi böngésző-eszközökkel valódi érintő-gesztus nem tesztelhető (a mobil-emuláció is egér-eseményeket küld), a recharts saját kereszt-diagram szinkronjára többé nem lehet megbízhatóan építeni — a kurzort ezért teljesen kivettem a recharts kezéből. A 3 diagram most egy közös, saját `cursorX` állapotot tart, amit bármelyik diagram saját `onMouseMove`/`onTouchMove` eseménye frissít (a recharts ezekben pontosan kiszámolt pixel-x-et ad, egérre és érintésre egyaránt) — egy új `CursorOverlay` komponens rajzolja a vonalat mindhárom diagram fölé, függetlenül attól, hogy melyiket érintik/hoverelik. Ez a mechanizmus már nem függ a recharts saját, bizonytalanul viselkedő szinkron-logikájától.

Ezután Marci egy teljes újratervezést diktált a napi beviteli dobozhoz: a "checklist" cím eltűnik mobilon; a fejléc két sorra tagolva (szint+gyakorlat neve — kód nélkül —, majd "mai megtartás: X mp" + jobbra igazított "⋯" menü); egy új "Milyen napod volt?" alcím; a "Tünet időtartama" mező mostantól kattintásra nyíló legördülő (nem mindig-látható select), új, óránkénti bontású opciólistával (2 órától 24 óráig) — az alapértelmezett érték első kitöltéskor az állapotfelmérő válaszából, később a legutóbbi bejegyzésből jön; az intenzitás-csúszka változatlan; a "nem hajolós nap" átnevezve "nem hajolás"-ra, elválasztó csíkkal leválasztva; az edzés-rögzítés gombpárja átalakult ("+ mai edzés" → "mai edzés hozzáadva"/"mai N edzés hozzáadva", mellette egyszerű "+" gomb a további edzésekhez), és minden edzéshez egy "közben '...'" kattintható, mondatként megfogalmazott tünet-válasz tartozik. Ehhez egy új, általános `ClickToSelect` komponenst építettem (a `HoldSecondsEditor` bevált mintáját követve), amit mind a tünet-időtartam, mind az edzésenkénti tünet-válasz mezők használnak.

**Tesztelve böngészőben:** valódi ÜF-bejelentkezéssel megerősítve, hogy a fejléc egy sorban mutatja a gyakorlat nevét (kód nélkül), a "Tünet időtartama" mező helyesen nyílik/zárul kattintásra, az edzés-gombok felirata és a "közben '...'" mondatok helyesen frissülnek. Mobilnézetben (375×812) a "checklist" cím eltűnt, a teljes doboz (2 edzéssel is) bőven görgetés nélkül fért el. A kurzor-szinkron egeres méréssel megerősítve (mindhárom diagram overlay-je azonos pixelen jelenik meg) — valódi érintő-gesztussal a jelenlegi eszközökkel nem tesztelhető. A GYT-oldali nézet változatlanul mutatja az S01 kódot. Világos és sötét módban is rendben, `npm run build`/`tsc -b` hibamentesek.
