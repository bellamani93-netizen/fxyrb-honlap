...
\- **Pontosítva (2026.08.25., véglegesítve ugyaneznap):** a chevron a kódban SVG-komponens (nem szöveges "«" karakter), tömör kitöltésű, éles sarkú (nem lekerekített) forma — pontosan a logó "BACK«««" nyilainak geometriáját követi. Magassága mindig `1em`. Egyetlen egységként (`<Chevron/>`) vagy két szorosan egymás mellé rakott egységként (`<Chevron double/>`, a logó ismétlődő mintáját idézve) használható. Iránya prop-pal állítható: alapértelmezett balra, `direction="right"` (scaleX tükrözés) és `direction="down"` (elforgatás). **Elhelyezés:** eyebrow-knál mindig a szöveg UTÁN áll (a logó "FIX YOUR BACK ‹‹‹‹" mintáját követve), nem elé.  
